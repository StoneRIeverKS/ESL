...
for(p in 1:P){
  ...
  for(k in 1:K){
    ...
    for(d in 1:datasize){
      ...
      Y[d] = X[d,1]+rnorm(n=1, mean=0, sd=1)
    }#d
...
#########################図の作成######################
png("次元数EPE.png", height=500, width=500)
plot(1:P, EPE_1, type="b", col="red", ylim=c(0.8, 2.5), xlab="次元数", ylab="EPE")
par(new=T)
plot(1:P, EPE_Linear, type="b", col="blue", ylim=c(0.8, 2.5), xlab="", ylab="")
dev.off()
